...
{
    LOG_TRACE << conn->name() << " " << conn->peerAddress().toIpPort() << " -> " << conn->localAddress().toIpPort()
...
void serverMessageCallback(const TcpConnectionPtr &conn, Buffer *buffer, xnet::Timestamp receiveTime)
{
    int64_t message[2];
...
void clientMessageCallback(const TcpConnectionPtr &, Buffer *buffer, xnet::Timestamp receiveTime)
{
    int64_t message[2];
    while (buffer->readableBytes() >= frameLen)
    {
