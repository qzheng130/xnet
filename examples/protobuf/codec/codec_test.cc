...
void testQuery()
{
    ...
    std::shared_ptr<xnet::Query> newQuery = down_pointer_cast<xnet::Query>(message);
...
void testAnswer()
{
    ...
    std::shared_ptr<xnet::Answer> newAnswer = down_pointer_cast<xnet::Answer>(message);
...
{
    int32_t checkSum = sockets::hostToNetwork32(
        static_cast<int32_t>(::adler32(1, reinterpret_cast<const Bytef *>(data.c_str()), static_cast<int>(len - 4))));
...
void onMessage(const xnet::net::TcpConnectionPtr &conn, const MessagePtr &message, xnet::Timestamp receiveTime)
{
    g_count++;
}
