...
struct RawMessage
{
    ...
    bool parse(const string &tag)
    {
        ...
        if (ProtobufCodecLite::validateChecksum(body, bodylen) && (memcmp(body, tag.data(), tag.size()) == 0) &&
            (bodylen >= taglen + 3 + 8))
        {
...
public:
    ...
    , nextId_(0)
    {
        ...
        client_.setMessageCallback(std::bind(&RpcCodec::onMessage, &codec_, _1, _2, _3));
        client_.enableRetry();
    }
    ...
    template <typename MSG> bool send(MSG &msg, const TcpConnectionPtr &clientConn)
    {
...
private:
    void sendTo(const TcpConnectionPtr &conn, const RpcMessage &msg) { codec_.send(conn, msg); }
    ...
    {
        loop_->assertInLoopThread();
        LOG_INFO << "Backend " << conn->localAddress().toIpPort() << " -> " << conn->peerAddress().toIpPort() << " is "
    ...
    void onRpcMessage(const TcpConnectionPtr &, const RpcMessagePtr &msg, Timestamp) { onMessageT(*msg); }

    bool onRawMessage(const TcpConnectionPtr &, StringPiece message, Timestamp)
    {
    ...
    template <typename MSG> void onMessageT(MSG &msg)
    {
        loop_->assertInLoopThread();
...
public:
    ...
    , codec_(std::bind(&Balancer::onRpcMessage, this, _1, _2, _3), std::bind(&Balancer::onRawMessage, this, _1, _2, _3))
    , backends_(backends)
    {
        server_.setThreadInitCallback(std::bind(&Balancer::initPerThread, this, _1));
        server_.setConnectionCallback(std::bind(&Balancer::onConnection, this, _1));
        server_.setMessageCallback(std::bind(&RpcCodec::onMessage, &codec_, _1, _2, _3));
    }
...
private:
    ...
    void onConnection(const TcpConnectionPtr &conn)
    {
        LOG_INFO << "Client " << conn->peerAddress().toIpPort() << " -> " << conn->localAddress().toIpPort() << " is "
                 << (conn->connected() ? "UP" : "DOWN");
        if (!conn->connected())
        {
    ...
    bool onRawMessage(const TcpConnectionPtr &conn, StringPiece message, Timestamp)
    {
        RawMessage raw(message);
        if (raw.parse(codec_.tag()))
        {
    ...
    void onRpcMessage(const TcpConnectionPtr &conn, const RpcMessagePtr &msg, Timestamp) { onMessageT(conn, *msg); }

    template <typename MSG> bool onMessageT(const TcpConnectionPtr &conn, MSG &msg)
    {
        PerThread &t = t_backends_.value();
        bool succeed = false;
