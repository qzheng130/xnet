...
{
public:
    Piece(const curl::RequestPtr &req, const FilePtr &out, const xnet::string &range, std::function<void()> done)
    : req_(req)
    , out_(out)
    , range_(range)
    , doneCb_(std::move(done))
    {
        ...
        req->setRange(range);
        req_->setDataCallback(std::bind(&Piece::onData, this, _1, _2));
        req_->setDoneCallback(std::bind(&Piece::onDone, this, _1, _2));
    }

private:
    void onData(const char *data, int len) { ::fwrite(data, 1, len, get_pointer(out_)); }
    ...
    void onHeaderDone(curl::Request *c, int code)
    {
        ...
        else if (found_)
        {
            ...
            if (fp)
            {
                ...
                req2_->setDataCallback(std::bind(&Downloader::onData, this, _1, _2));
                req2_->setDoneCallback(std::bind(&Downloader::onDownloadDone, this));
                concurrent_ = 1;
            }
            else
            {
    ...
    void concurrentDownload()
    {
        ...
        for (int i = 0; i < kConcurrent; ++i)
        {
            ...
            if (fp)
            {
                ...
                pieces_[i].reset(new Piece(req, out, range.str(), std::bind(&Downloader::onDownloadDone, this)));
            }
            else
            {
