...
xnet::string Plot::plotCpu(const std::vector<double> &data)
{
    ...
    if (data.size() > 1)
    {
        ...
        {
            gdImageLine(image_, getX(i, data.size()), getY(data[i] / max), getX(i + 1, data.size()),
                        getY(data[i + 1] / max), black_);
        }
    }
...
void Plot::label(double maxValue)
{
    ...
    gdImageString(image_, font_, width_ - kRightMargin_ + 3, kMarginY_ - 3, reinterpret_cast<unsigned char *>(buf),
                  black_);

    snprintf(buf, sizeof buf, "0");
    gdImageString(image_, font_, width_ - kRightMargin_ + 3, height_ - kMarginY_ - 3 - fontHeight_ / 2,
...
xnet::string Plot::toPng()
{
    int size = 0;
    void *png = gdImagePngPtr(image_, &size);
    xnet::string result(static_cast<char *>(png), size);
    gdFree(png);
    return result;
}
