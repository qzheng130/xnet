...
struct StatData
{
    void parse(const char *startAtState, int kbPerPage)
    {
        ...
        // see xnet-protorpc/examples/collect/ProcFs.cc for alternatives.
...
public:
    ...
    : kClockTicksPerSecond_(xnet::ProcessInfo::clockTicksPerSecond())
    , kbPerPage_(xnet::ProcessInfo::pageSize() / 1024)
...
private:
    ...
    void onRequest(const HttpRequest &req, HttpResponse *resp)
    {
        ...
        resp->addHeader("Server", "XNet-Procmon");
        ...
        for (int i = 0; i < result; ++i)
        {
            ...
            if (::lstat(namelist[i]->d_name, &stat) == 0)
            {
                ...
                appendResponse("%c %9" PRId64 " %s %s", getDirType(namelist[i]->d_type), size,
                               mtime.toFormattedString(/*showMicroseconds=*/false).c_str(), namelist[i]->d_name);
    ...
    string getCmdLine() { return boost::replace_all_copy(readProcFile("cmdline"), string(1, '\0'), "\n\t"); }
    ...
    double getSeconds(long ticks) { return static_cast<double>(ticks) / kClockTicksPerSecond_; }
