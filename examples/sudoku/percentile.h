...
public:
    ...
    {
        stat << "recv " << xnet::Fmt("%6zd", latencies.size()) << " in-fly " << infly;
        ...
        if (!latencies.empty())
        {
            ...
            stat << " min " << min << " max " << max << " avg " << mean << " median " << median << " p90 " << p90
                 << " p99 " << p99;
        }
    }

    const xnet::LogStream::Buffer &report() const { return stat.buffer(); }

    void save(const std::vector<int> &latencies, xnet::StringArg name) const
    {
        if (latencies.empty())
            return;
        xnet::FileUtil::AppendFile f(name);
        f.append("# ", 2);
