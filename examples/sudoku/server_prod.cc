...
{
public:
    SudokuServer(EventLoop *loop, const InetAddress &listenAddr, int numEventLoops, int numThreads, bool nodelay)
    ...
    , inspector_(inspectThread_.startLoop(), InetAddress(9982), "sudoku-solver")
    {
        ...
        inspector_.add("sudoku", "stats", std::bind(&SudokuStat::report, &stat_), "statistics of sudoku solver");
        inspector_.add("sudoku", "reset", std::bind(&SudokuStat::reset, &stat_), "reset statistics of sudoku solver");
    }
...
private:
    void onConnection(const TcpConnectionPtr &conn)
    {
        ...
        if (conn->connected())
        {
            ...
            conn->setHighWaterMarkCallback(std::bind(&SudokuServer::highWaterMark, this, _1, _2), 5 * 1024 * 1024);
            bool throttle = false;
    ...
    void highWaterMark(const TcpConnectionPtr &conn, size_t tosend)
    {
        ...
        {
            conn->setHighWaterMarkCallback(std::bind(&SudokuServer::highWaterMark, this, _1, _2), 10 * 1024 * 1024);
