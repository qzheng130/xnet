...
{
public:
    SudokuClient(EventLoop *loop, const InetAddress &serverAddr, const InputPtr &input, const string &name,
    ...
    , count_(0)
    {
        client_.setConnectionCallback(std::bind(&SudokuClient::onConnection, this, _1));
...
{
    LOG_INFO << name << " " << elapsed << " seconds " << Fmt("%.3f", 1000 * 1000 * elapsed / count)
             << " us per request.";
    ++g_finished;
    if (g_finished == g_connections)
    {
        ...
        LOG_INFO << "total " << total << " seconds, " << (total / g_connections) << " seconds per client";
    }
}
