#ifndef XNET_EXAMPLES_MEMCACHED_SERVER_SESSION_H
#define XNET_EXAMPLES_MEMCACHED_SERVER_SESSION_H

#include "examples/memcached/server/Item.h"

#include "xnet/base/Logging.h"

#include "xnet/net/TcpConnection.h"

#include <boost/tokenizer.hpp>

using xnet::string;

class MemcacheServer;

class Session : public std::enable_shared_from_this<Session>, xnet::noncopyable
{
public:
    Session(MemcacheServer *owner, const xnet::net::TcpConnectionPtr &conn)
    : owner_(owner)
    , conn_(conn)
    , state_(kNewCommand)
    , protocol_(kAscii)
    , // FIXME
    noreply_(false)
    , policy_(Item::kInvalid)
    , bytesToDiscard_(0)
    , needle_(Item::makeItem(kLongestKey, 0, 0, 2, 0))
    , bytesRead_(0)
    , requestsProcessed_(0)
    {
        using std::placeholders::_1;
        using std::placeholders::_2;
        using std::placeholders::_3;

        conn_->setMessageCallback(std::bind(&Session::onMessage, this, _1, _2, _3));
    }

    ~Session()
    {
        LOG_INFO << "requests processed: " << requestsProcessed_
                 << " input buffer size: " << conn_->inputBuffer()->internalCapacity()
                 << " output buffer size: " << conn_->outputBuffer()->internalCapacity();
    }

private:
    enum State
    {
        kNewCommand,
        kReceiveValue,
        kDiscardValue,
    };

    enum Protocol
    {
        kAscii,
        kBinary,
        kAuto,
    };

    void onMessage(const xnet::net::TcpConnectionPtr &conn, xnet::net::Buffer *buf, xnet::Timestamp);
    void onWriteComplete(const xnet::net::TcpConnectionPtr &conn);
    void receiveValue(xnet::net::Buffer *buf);
    void discardValue(xnet::net::Buffer *buf);
    // TODO: highWaterMark
    // TODO: onWriteComplete

    // returns true if finished a request
    bool processRequest(xnet::StringPiece request);
    void resetRequest();
    void reply(xnet::StringPiece msg);

    struct SpaceSeparator
    {
        void reset() { }

        template <typename InputIterator, typename Token>
        bool operator()(InputIterator &next, InputIterator end, Token &tok);
    };

    typedef boost::tokenizer<SpaceSeparator, const char *, xnet::StringPiece> Tokenizer;
    struct Reader;
    bool doUpdate(Tokenizer::iterator &beg, Tokenizer::iterator end);
    void doDelete(Tokenizer::iterator &beg, Tokenizer::iterator end);

    MemcacheServer *owner_;
    xnet::net::TcpConnectionPtr conn_;
    State state_;
    Protocol protocol_;

    // current request
    string command_;
    bool noreply_;
    Item::UpdatePolicy policy_;
    ItemPtr currItem_;
    size_t bytesToDiscard_;
    // cached
    ItemPtr needle_;
    xnet::net::Buffer outputBuf_;

    // per session stats
    size_t bytesRead_;
    size_t requestsProcessed_;

    static string kLongestKey;
};

typedef std::shared_ptr<Session> SessionPtr;

#endif // XNET_EXAMPLES_MEMCACHED_SERVER_SESSION_H
