...
void Session::onMessage(const xnet::net::TcpConnectionPtr &conn, xnet::net::Buffer *buf, xnet::Timestamp)
...
void Session::receiveValue(xnet::net::Buffer *buf)
{
...
void Session::discardValue(xnet::net::Buffer *buf)
{
    assert(!currItem_);
...
bool Session::processRequest(StringPiece request)
{
    ...
    ++beg;
    if (command_ == "set" || command_ == "add" || command_ == "replace" || command_ == "append" ||
        command_ == "prepend" || command_ == "cas")
    {
    ...
    {
#ifdef HAVE_TCMALLOC
        reply("VERSION 0.01 xnet with tcmalloc\r\n");
#else
        reply("VERSION 0.01 xnet\r\n");
#endif
    }
#ifdef HAVE_TCMALLOC
...
void Session::reply(xnet::StringPiece msg)
{
    if (!noreply_)
    {
