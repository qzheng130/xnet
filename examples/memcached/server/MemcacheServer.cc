...
xnet::AtomicInt64 g_cas;

MemcacheServer::Options::Options() { memZero(this, sizeof(*this)); }

struct MemcacheServer::Stats
{
};

MemcacheServer::MemcacheServer(xnet::net::EventLoop *loop, const Options &options)
: loop_(loop)
, options_(options)
, startTime_(::time(NULL) - 1)
, server_(loop, InetAddress(options.tcpport), "xnet-memcached")
, stats_(new Stats)
{
...
void MemcacheServer::stop() { loop_->runAfter(3.0, std::bind(&EventLoop::quit, loop_)); }
    ...
    else
    {
        ...
        else if (policy == Item::kAppend || policy == Item::kPrepend)
        {
            if (*exists)
            {
                ...
                ItemPtr newItem(Item::makeItem(item->key(), oldItem->flags(), oldItem->rel_exptime(), newLen,
