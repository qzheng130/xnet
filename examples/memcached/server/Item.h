#ifndef XNET_EXAMPLES_MEMCACHED_SERVER_ITEM_H
#define XNET_EXAMPLES_MEMCACHED_SERVER_ITEM_H
...
public:
    ...
    static ItemPtr makeItem(xnet::StringPiece keyArg, uint32_t flagsArg, int exptimeArg, int valuelen, uint64_t casArg)
    {
    ...
    Item(xnet::StringPiece keyArg, uint32_t flagsArg, int exptimeArg, int valuelen, uint64_t casArg);

    ~Item() { ::free(data_); }

    xnet::StringPiece key() const { return xnet::StringPiece(data_, keylen_); }
    ...
    void output(xnet::net::Buffer *out, bool needCas = false) const;

    void resetKey(xnet::StringPiece k);
...
#endif // XNET_EXAMPLES_MEMCACHED_SERVER_ITEM_H
