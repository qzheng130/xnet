...
public:
    ...
    , latch_(numThreads)
    {
        ...
        for (int i = 0; i < numThreads; ++i)
        {
            ...
            threads_.emplace_back(new xnet::Thread(std::bind(&Test::threadFunc, this), xnet::string(name)));
        }
    ...
    void run(int times)
    {
        ...
        for (int i = 0; i < times; ++i)
        {
            ...
            queue_.put(buf);
            printf("tid=%d, put data = %s, size = %zd\n", xnet::CurrentThread::tid(), buf, queue_.size());
        }
    }
...
private:
    void threadFunc()
    {
        ...
        while (running)
        {
            ...
            printf("tid=%d, get data = %s, size = %zd\n", xnet::CurrentThread::tid(), d.c_str(), queue_.size());
            running = (d != "stop");
        }
    ...
    xnet::BoundedBlockingQueue<std::string> queue_;
...
void testMove()
{
    ...
    xnet::BoundedBlockingQueue<std::unique_ptr<int>> queue(10);
