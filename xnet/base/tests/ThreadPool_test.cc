...
void test(int maxSize)
{
    ...
    pool.run(std::bind(&xnet::CountDownLatch::countDown, &latch));
    latch.wait();
    pool.stop();
}
...
void testMove()
{
  xnet::ThreadPool pool;
  pool.start(2);

  std::unique_ptr<int> x(new int(42));
  pool.run([y = std::move(x)]{ printf("%d: %d\n", xnet::CurrentThread::tid(), *y); });
  pool.stop();
}
*/
...
void longTask(int num)
{
    ...
    xnet::CurrentThread::sleepUsec(3000000);
}

void test2()
{
    LOG_WARN << "Test ThreadPool by stoping early.";
    xnet::ThreadPool pool("ThreadPool");
    ...
    xnet::CurrentThread::sleepUsec(5000000);
    LOG_WARN << "stop pool";
