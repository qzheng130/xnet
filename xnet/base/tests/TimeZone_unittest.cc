...
void test(const TimeZone &tz, TestCase tc)
{
...

    {
        ...
        DateTime localtime(local.tm_year + 1900, local.tm_mon + 1, local.tm_mday, local.tm_hour, local.tm_min,
                           local.tm_sec);
        ...
        if (result != gmt)
        {
            ...
            printf("WRONG fromLocalTime: input %s expect %s got %s\n", tc.local, tc.gmt,
                   tz.toUtcTime(result).toIsoString().c_str());
        }
    }
}
        ...
        if (utc != gmt)
        {
            ...
            if (post != utc)
            {
                ...
                printf("WRONG: input %s local %s got %s\n", tz.toUtcTime(utc).toIsoString().c_str(),
                       local.toIsoString().c_str(), tz.toUtcTime(gmt).toIsoString().c_str());
            }
        }
    }
}
