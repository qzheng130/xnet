...
xnet::MutexLock g_mutex;
xnet::AtomicInt32 g_count;
...
void forkBench()
{
    ...
    double timeUsed = timeDifference(xnet::Timestamp::now(), start);
...
public:
    ...
    , stopLatch_(1)
    {
        ...
        for (int i = 0; i < numThreads; ++i)
        {
            ...
            threads_.emplace_back(new xnet::Thread([this] { threadFunc(); }, xnet::string(name)));
        }
    }
...
private:
    void threadFunc()
    {
        const int tid = xnet::CurrentThread::tid();
        start_.put(std::make_pair(tid, xnet::Timestamp::now()));
        startLatch_.countDown();
        stopLatch_.wait();
        done_.put(std::make_pair(tid, xnet::Timestamp::now()));
    }
...
{
    g_verbose = argc > 1;
    printf("pid=%d, tid=%d, verbose=%d\n", ::getpid(), xnet::CurrentThread::tid(), g_verbose);
    xnet::Timestamp start(xnet::Timestamp::now());
    ...
    printf("number of created threads %d, g_count = %d\n", xnet::Thread::numCreated(), g_count.get());

    for (int i = 0; i < kThreads; ++i)
    {
        xnet::Timestamp now(xnet::Timestamp::now());
        xnet::Thread t2(std::bind(threadFunc2, now));
        t2.start();
        t2.join();
    }
